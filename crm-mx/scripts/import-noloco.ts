/**
 * Import idempotente de Noloco → CRM (fases 1-2 del plan).
 *
 ...
 * - GATE: los conteos mensuales 2026 de casos nuevos deben coincidir con los reportes
 *   de Juan o el import aborta sin escribir.
 */
...
async function main() {
  ...
  type DocAgg = {
    ...
    phone: string | null;
    categoria: string;
    allDates: string[];    // fechaIngreso de todos los casos
    newDates: string[];    // solo I_1
  };
  ...
  console.log(`Doctores únicos: ${docs.size}`);
  ...
  // paginado obligatorio: sin esto PostgREST devuelve 1.000 de 6.4k doctores y
  ...
  console.log(`Casos upserted: ${caseRows.length}`);
  ...
  console.log("Objetivos H2 sembrados (rampa 24→30)");

  // ---------- recompute total (cohortes + scores) ----------
  const { error: rcErr } = await db.rpc("recompute_all");
  if (rcErr) console.warn("recompute_all falló (¿migración 0005 aplicada?):", rcErr.message);
  else console.log("Scores recalculados ✓");
  ...
  const { count: newCount } = await db
    ...
    .eq("is_new_case", true);
  console.log(`Casos nuevos (I_1) en DB: ${newCount}`);
